...
- Название сборки через `androidAppName`, а runtime-заголовок подтягивается из `/api/cabinet/branding`.
...
cabinetUrl=https://your-domain.example/
versionCode=1
versionName=1.0.0
...
Если кабинет опубликован не в корне домена, укажите полный путь:
...
gradle :android-app:assembleDebug `
  ...
  -PcabinetUrl=https://your-domain.example/
```

## Keystore

Генерацию ключа лучше сделать в админ-панели отдельным этапом. До этого можно создать локальный тестовый ключ:
...
## Что вынесено на следующий этап

- Firebase push-уведомления.
- Android App Links с `assetlinks.json`.
- Смена домена внутри приложения, если нужно разрешить это владельцам.
